...
// // Função para processar o CSV e gerar LaTeX
...
//         //Métricas contínuas
//         lcc: { bom: 0.6044, ruim: 1 },
//         lcom: { bom: 0.4, ruim: 0.7917 },
//         tcc: { bom: 0.4483, ruim: 1 }
//     };
...
//         if (metric === 'lcc' || metric === 'lcom' || metric === 'tcc') {
...
//     // Gerar tabela LaTeX
//     let latex = `\\begin{tabular}{lccc}
//                 \\toprule
//                 \\textbf{Métrica} & \\textbf{Bom (\\%)} & \\textbf{Regular (\\%)} & \\textbf{Ruim (\\%)} \\\\
//                 \\midrule
//                 `;
//     numericColumn.forEach(column => {
//         const total = result[column].Total;
//         if (total > 0) {
//             const bom = ((result[column].Bom / total) * 100).toFixed(1);
//             const regular = ((result[column].Regular / total) * 100).toFixed(1);
//             const ruim = ((result[column].Ruim / total) * 100).toFixed(1);
//             latex += `${column} & ${bom} & ${regular} & ${ruim} \\\\\n`;
//         }
//     });
...
//     fs.writeFileSync('tabelaResultados.tex', latex, 'utf-8');
//     console.log('Tabela LaTeX gerada em tabelaResultados.tex');
// }
